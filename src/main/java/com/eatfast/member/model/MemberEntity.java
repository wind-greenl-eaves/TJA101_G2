...
/**
 * 會員資料實體 (Entity)
 * 儲存前台使用者的所有資訊，對應資料庫中的 'member' 資料表。
 */
@Entity
@Table(name = "member",
    uniqueConstraints = {
...
public class MemberEntity {
    ...
    /**
     * 登入帳號 (使用者自訂，不可重複)
     * 為業務主鍵 (Business Key)，用於判斷物件邏輯相等性。
     */
     ...
     * @param birthday 生日
     */
    public MemberEntity(String username, String account, String password, String email, String phone, LocalDate birthday) {
     ...
     * 例如 `account` 和 `email`，因為它們在業務上具有唯一性。
     * 如此可確保無論物件是否已存入資料庫，比較邏輯都保持一致。
     */
    @Override
